...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller must guarantee that `address` is a valid, unique, mutable pointer to a `u32` value.
    unsafe {
        let ptr = address as *mut u32;
        // SAFETY: The pointer is assumed valid and points to a mutable u32, as required by the function contract.
        *ptr = 0xAABBCCDD;
    }
}
